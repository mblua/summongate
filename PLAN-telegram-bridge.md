# Plan: Telegram Bridge para agentscommander
...
| `src-tauri/src/commands/session.rs` | Auto-attach: leer `.agentscommander/config.json` al crear sesión |
...
// Per-repo config (.agentscommander/config.json)
...
- `.agentscommander/config.json` → `{ "telegramBot": "mi-bot" }`
...
15. Leer `.agentscommander/config.json` en create_session
...
1. Agregar un bot en Settings, guardar, recargar → persiste en `~/.agentscommander/settings.json`
...
6. Crear `.agentscommander/config.json` en un repo, abrir sesión ahí → auto-attach
